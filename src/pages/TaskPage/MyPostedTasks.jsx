...
const MyPostedTasks = () => {
    ...
    useEffect(() => {
        if (user?.email) {
            ...
            setTaskLoading(false); // Prevent indefinite loading if user is not logged in
        }
    }, [user]);
    ...
    if (authLoading || taskLoading) return <Loading />;

    if (!user) {
        return <div className="text-center py-10 text-lg text-red-600">Please log in to view your posted tasks.</div>;
    }

    return (
        <div className="max-w-7xl md:h-[80vh] mx-auto px-4 py-8 md:my-12">
